...
RSpec.describe 'rake provisioner:up' do
  ...
  let!(:up_instance) do
    Provisioner::Commands::Up.new(client)
  end

  before do
    allow(Aws::ECS::Client).to receive(:new).and_return(client)
    allow(Provisioner::Commands::Up).to receive(:new).and_return(up_instance)
  end
  ...
  it 'logs the cluster, task_definition and service created' do
    allow(Provisioner.logger).to receive(:info)
    ...
    expect(Provisioner.logger).to have_received(:info).with(result)
  end
  ...
  context 'when params defined in the JSON file are not valid' do
    ...
    it 'logs errors' do
      allow(Provisioner.logger).to receive(:error)
      ...
      expect(Provisioner.logger).to have_received(:error).with(result)
    end
  end

  context 'when filepath argument is missing' do
    it 'logs error' do
      expect(Provisioner.logger).to receive(:error).with(/filepath is missing/)
  ...
  context 'when path to JSON file does not exist' do
    it 'logs error' do
      expect(Provisioner.logger).to receive(:error).with(/filepath not found/)
  ...
  context 'when file is not JSON formatted' do
    ...
    it 'logs error' do
      expect(Provisioner.logger).to receive(:error).with(/malformed JSON/)
