...
RSpec.describe Provisioner::Commands::Up do
  let(:client) do
  ...
  describe '#call' do
    ...
    context 'Aws::ECS::Service resource type' do
      ...
      it 'service uses the cluster arn for the referenced logical cluster name' do
        expect_any_instance_of(Provisioner::Commands::CreateService).to receive(:call).with(include(cluster: 'a_cluster_arn')).and_call_original

        subject.call(default_params)
      end

      it 'service uses the task definition arn for the referenced logical task definition name' do
        expect_any_instance_of(Provisioner::Commands::CreateService).to receive(:call).with(include(task_definition: 'a_task_definition_arn')).and_call_original
