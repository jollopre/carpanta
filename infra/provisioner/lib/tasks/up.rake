  ...
  task :up, [:filepath] do |t, args|
    ...
    klass = Class.new do
      ...
      def call(args)
        ...
        client = Aws::ECS::Client.new(logger: Provisioner.logger)
        up = Provisioner::Commands::Up.new(client)
      ...
      class << self
        def call(args = {})
          ...
          Dry::Matcher::ResultMatcher.call(result) do |m|
            m.success do |v|
              Provisioner.logger.info(v.to_json)
            end
            m.failure do |v|
              Provisioner.logger.error(v.to_json)
            end
          end
        end
      end
