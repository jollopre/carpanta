...
RSpec.describe Carpanta::Domain::Offers::OfferLegacy do
  describe '.build' do
    it 'returns an offer' do
      offer = FactoryBot.build(:offer_legacy)
  ...
  describe 'tasks' do
    it 'needs to be present' do
      offer = FactoryBot.build(:offer_legacy, tasks: nil)
    ...
    it 'needs to be an array' do
      offer = FactoryBot.build(:offer_legacy, tasks: 'foo')
    ...
    context 'when it is an array' do
      context 'but any of its objects is not string' do
        it 'returns error, value and index' do
          offer = FactoryBot.build(:offer_legacy, tasks: ['foo', nil, {}])
  ...
  describe 'price' do
    it 'needs to be present' do
      offer = FactoryBot.build(:offer_legacy, price: nil)
    ...
    context 'numericality' do
      it 'needs to be integer' do
        offer = FactoryBot.build(:offer_legacy, price: 22.00)
      ...
      it 'needs to be positive' do
        offer = FactoryBot.build(:offer_legacy, price: -2200)
      ...
      it 'needs to be greater than zero' do
        offer = FactoryBot.build(:offer_legacy, price: 0)
  ...
  describe '#name' do
    ...
    context 'when there is one element' do
      it 'does not add the connector' do
        offer = FactoryBot.build(:offer_legacy, tasks: ['Cutting with scissor'])
